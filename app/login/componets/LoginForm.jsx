...
export default function SignUpForm() {
    ...
    const handleSubmit = async(e) => {
        ...
        try {
            ...
            setIsLoading(false);
            setCanProceed(false);
            setErrorMessage(t('signup.error_message'));
    ...
    const createAccountHandler = (e) => {
        ...
        toast.promise(
            promise,
            {
                loading: t('signup.setting_up'),
                error: t('signup.registration_failed'),
    ...
    useEffect(() => {
        if (debouncedEmail !== "") {
            if (existingEmail.includes(String(debouncedEmail).toLowerCase())) {
                ...
                setErrorMessage(t('signup.email_exists'));
                return;
            }

            if (!validateEmail(debouncedEmail)) {
                setHasError((prevData) => ({ ...prevData, email: 1 }));
                setErrorMessage(t('signup.invalid_email'));
                return;
            }
    ...
    return (
        <div className="flex-1 sm:p-12 py-8 p-2 flex flex-col overflow-y-auto">
            ...
            <section className="mx-auto py-10 w-full sm:w-5/6 md:w-3/4 lg:w-2/3 xl:w-1/2 flex-1 flex flex-col justify-center">
                ...
                <p className="text-center"><span className="opacity-60">{t('signup.have_account')}</span> <Link className="text-themeGreen" href={"/login"}>{t('signup.login')}</Link> </p>
            </section>
        </div>
    )
}